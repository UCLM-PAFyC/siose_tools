...
class ClipSioseHrTask(QgsTask):
    ...
    def run(self):
        ...
        output_name = 'clip_siose'
        target_path = os.path.dirname(self.target_file_name)
        o_std = open(os.path.join(target_path, '{}.log'.format(output_name)), 'w')
        o_err = open(os.path.join(target_path, '{}.err'.format(output_name)), 'w')
        start = datetime.now()
        if not self.from_administrative_unit:
            ...
            if self.load_as_process:
                ...
                command = [ogr2ogr_with_path, str_target_file_name,
                ...
                try:
                    subprocess.check_call(str_command, stdout=o_std, stderr=o_err, shell=True)
            ...
            command = [ogrinfo_with_path, "-sql", sql_statment_str, str_target_file_name]
            str_command = ''
            for str_aux in command:
                str_command += str_aux + ' '
            # self.display_msg_error(str_command)
            ...
            command = [ogrinfo_with_path, "-sql", "\"VACUUM\"", str_target_file_name]
            str_command = ''
            for str_aux in command:
                str_command += str_aux + ' '
            # self.display_msg_error(str_command)
